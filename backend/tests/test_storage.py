...
def test_save_and_retrieve_pdf(tmp_path, monkeypatch):
    monkeypatch.setattr("app.db.settings.storage_dir", str(tmp_path))

    data = b"%PDF-1.4 fake content"
    save_pdf("book-123", data)

    assert get_pdf_path("book-123").exists()
    assert get_pdf_path("book-123").read_bytes() == data


def test_delete_pdf(tmp_path, monkeypatch):
    monkeypatch.setattr("app.db.settings.storage_dir", str(tmp_path))
...
def test_delete_nonexistent_pdf_is_noop(tmp_path, monkeypatch):
    monkeypatch.setattr("app.db.settings.storage_dir", str(tmp_path))
